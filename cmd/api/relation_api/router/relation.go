...
func InitRelationRouter(Router *gin.RouterGroup)  {//注册用户相关的路由
	...
	{
		RelationRouter.POST("/action",middlewares.JWTAuth(global2.ServerConfig.JWTInfo.SigningKey),
			handlers2.RelationAction) //先验证,再给Info
		RelationRouter.GET("/follow_list",middlewares.JWTAuth(global2.ServerConfig.JWTInfo.SigningKey),
			handlers2.RelationFollowList) //先验证,再给Info
		RelationRouter.GET("/follower_list",middlewares.JWTAuth(global2.ServerConfig.JWTInfo.SigningKey),
